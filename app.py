...
if not st.session_state.usuario_id:
    ...
    if st.button("Entrar"):
        data = run_query("SELECT id,password FROM usuarios WHERE usuario=%s",
                         (usuario,), True)
    ...
    if st.button("Crear usuario"):
        if usuario and password:
            ...
            run_query("INSERT INTO usuarios (usuario,password) VALUES (%s,%s)",
                      (usuario, hashp))
    ...
    # DATA
    # =========================
    data = run_query("SELECT * FROM movimientos WHERE usuario_id=%s", (uid,), True)
    df = pd.DataFrame(data, columns=["id","usuario_id","fecha","tipo","categoria","monto"]) if data else pd.DataFrame()
    ...
    if menu == "Dashboard":
        ...
        if not df.empty:
            ...
            st.metric("Ahorro", ingresos-gastos)

            # 🔮 Predicción global
            hoy = pd.Timestamp.today()
            gastos_mes = df[(df.tipo=="Gasto") & (df.mes==hoy.to_period("M"))]
            ...
            pred = {c:(m/hoy.day)*dias_mes for c,m in gasto_cat.items()} if hoy.day>0 else {}
    ...
    # =========================
        ...
        if not df.empty:
            if desde: df_f = df_f[df_f.fecha>=pd.to_datetime(desde)]
            if hasta: df_f = df_f[df_f.fecha<=pd.to_datetime(hasta)]
        ...
        if st.button("Eliminar"):
            run_query("DELETE FROM movimientos WHERE id=%s AND usuario_id=%s",(id_del,uid))
            st.rerun()

        # editar
        id_ed = st.number_input("ID editar", step=1)
        new_m = st.number_input("Nuevo monto")
        ...
        if not df_f.empty:
            ...
            st.download_button("Descargar Excel",buf.getvalue(),"datos.xlsx")
    ...
    # =========================
        ...
        cats = run_query("SELECT nombre FROM categorias WHERE usuario_id=%s",(uid,),True)
        st.write(cats)
    ...
    # =========================
        ...
        if st.button("Guardar presupuesto"):
            run_query("""
            INSERT INTO presupuestos(usuario_id,categoria,mes,monto)
            VALUES (%s,%s,%s,%s)
            """,(uid,cat,mes,monto))
            st.rerun()

        pres = run_query("SELECT categoria,monto FROM presupuestos WHERE usuario_id=%s AND mes=%s",
                         (uid,mes),True)

        if pres and not df.empty:

            gastos_mes = df[(df.tipo=="Gasto") & (df.mes==pd.Timestamp.today().to_period("M"))]
